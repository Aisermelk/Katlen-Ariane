...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Sobre (formação/especializações)
    if (config.formacao) document.getElementById("about-formacao").textContent = config.formacao;
    if (config.especializacoes) document.getElementById("about-especializacoes").textContent = config.especializacoes;
    ...
    const socialIds = { instagram: "instagram-link", facebook: "facebook-link", tiktok: "tiktok-link", linkedin: "linkedin-link" };
    Object.entries(socialIds).forEach(([key, id]) => {
        const el = document.getElementById(id);
        if (el && config[key]) { el.href = config[key]; el.style.display = "inline-block"; }
        else if (el) { el.style.display = "none"; }
    });
    const footerWhatsapp = document.getElementById("footer-whatsapp");
    if (footerWhatsapp && config.whatsapp) footerWhatsapp.textContent = `WhatsApp: ${config.whatsapp}`;
    const footerAddress = document.getElementById("footer-address");
    if (footerAddress) footerAddress.textContent = config.endereco || "";
    const footerRegistration = document.getElementById("footer-registration");
    if (footerRegistration && config.professionalRegistration) footerRegistration.textContent = `${config.professionalRegistrationLabel || "Registro profissional"}: ${config.professionalRegistration}`;
